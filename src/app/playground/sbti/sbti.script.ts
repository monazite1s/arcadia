...
interface SBTIType {
    ...
    intro: string;
    desc: string;
    specialType?: "special" | "fallback";
}

// ─── Data ─────────────────────────────────────────────────────────────────────
const IMG_BASE = "https://sbti.unun.dev/image/";
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function dominantGroup(pattern: string): string {
    if (!pattern) return "special";
    ...
    for (let i = 0; i < chars.length; i++) {
        const n = chars[i] === "H" ? 3 : chars[i] === "M" ? 2 : 1;
        scores[DIM_GROUPS[i]] += n;
    }
    return Object.entries(scores).sort(([, a], [, b]) => b - a)[0][0];
}

function patternHTML(pattern: string): string {
    if (!pattern) return '<span style="color:var(--sbti-muted)">无固定 Pattern（特殊触发）</span>';
    return pattern
        .split("")
        .map((ch) => {
            if (ch === "-") return '<span class="sbti-dim-sep">-</span>';
            const cls = ch === "H" ? "sbti-dim-h" : ch === "M" ? "sbti-dim-m" : "sbti-dim-l";
...
// ─── Render ───────────────────────────────────────────────────────────────────
export function renderSBTI(filter: string): void {
    const grid = document.getElementById("sbtiGrid");
    if (!grid) return;
    grid.innerHTML = "";

    types.forEach((t) => {
        const group = t.specialType || dominantGroup(t.pattern);
        if (filter !== "all" && group !== filter) return;

        let badgeHTML = "";
        if (t.specialType === "special") {
            badgeHTML = '<span class="sbti-badge sbti-badge--gold">隐藏人格 · 酒精触发</span>';
        } else if (t.specialType === "fallback") {
            badgeHTML =
                '<span class="sbti-badge sbti-badge--red">兜底人格 · 匹配度 &lt; 60%</span>';
        }

        const card = document.createElement("div");
        card.className = "sbti-card";
        card.innerHTML = `
      <div class="sbti-card__top">
        <img class="sbti-card__avatar" src="${IMG_BASE}${t.img}" alt="${esc(t.code)}" loading="lazy" onerror="this.style.visibility='hidden'" />
        <div class="sbti-card__head">
          <div><span class="sbti-card__code">${esc(t.code)}</span><span class="sbti-card__cn">${esc(t.cn)}</span></div>
          <div class="sbti-card__intro">${esc(t.intro)}</div>
        </div>
      </div>
      ${badgeHTML ? `<div class="sbti-card__badges">${badgeHTML}</div>` : ""}
      <div class="sbti-card__pattern">${t.pattern ? "维度 " + patternHTML(t.pattern) : patternHTML("")}</div>
      <div class="sbti-card__desc" id="desc-${t.code}">${esc(t.desc)}</div>
      <div class="sbti-card__actions">
        <button class="sbti-card__toggle" data-code="${t.code}">展开全文 ▼</button>
        <button class="sbti-card__preview" data-code="${t.code}">预览结果页</button>
      </div>
    `;
...
export function initSBTIFilter(): void {
    const filterBar = document.getElementById("sbtiFilter");
    if (!filterBar) return;
    ...
    filterBar.addEventListener("click", (e) => {
        ...
        renderSBTI(btn.dataset.f || "all");
    });

    // 展开/收起描述 + 预览结果页
    document.getElementById("sbtiGrid")?.addEventListener("click", (e) => {
        const target = e.target as HTMLElement;

        // 展开/收起
        const toggleBtn = target.closest(".sbti-card__toggle") as HTMLButtonElement | null;
        if (toggleBtn) {
        ...
        // 预览结果页
        const previewBtn = target.closest(".sbti-card__preview") as HTMLButtonElement | null;
        if (previewBtn) {
            const code = previewBtn.dataset.code;
            if (!code) return;
            window.dispatchEvent(new CustomEvent("sbti-preview", { detail: code }));
        }
    });
}
